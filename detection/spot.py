...
class SPOT:
    ...
    def run(self, with_alarm=True):
        ...
        for i in tqdm.tqdm(range(self.data.size)):
            ...
            # If the observed value exceeds the extreme quantile (alarm case)
...
class biSPOT:
    ...
    def run(self, with_alarm=True):
        ...
        for i in tqdm.tqdm(range(self.data.size)):
            ...
            # If the observed value exceeds the current threshold(extreme quantile) (alarm case)
            if self.data[i] > self.extreme_quantile['up'] :
                # if we want to alarm, we put it in the alarm list
                if with_alarm:
                    alarm.append(i)
                # otherwise we add it in the peaks (ignore the alarm)
                else:
            ...
            # case where the observed value lower than the current threshold(extreme quantile) (alarm case)
            ...
            th_up.append(self.extreme_quantile['up']) # thresholds record
            th_down.append(self.extreme_quantile['down']) # thresholds record
        
        return {'upper_thresholds' : th_up,'lower_thresholds' : th_down, 'alarms': alarm}
